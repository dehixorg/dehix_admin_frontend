    ...
    <CardContent>
      <Table className="w-full text-white bg-black  "> {/* Full width table with black background */}
        ...
        <TableBody>
          {loading ? (
            <TableRow>
              <TableCell colSpan={4} className="text-white text-center">Loading...</TableCell> 
            </TableRow>
          ) : hireFreelancers.length === 0 ? (
            <TableRow>
              <TableCell colSpan={4} className="text-white text-center">No freelancers found.</TableCell> 
            </TableRow>
          ) : (
            hireFreelancers.map((hireFreelancer, index) => (
              <TableRow key={hireFreelancer._id} className="border-b border-gray-700"> {/* Add border styling */}
                <TableCell className="py-2">{index + 1}</TableCell> {/* Serial number */}
                <TableCell className="py-2">{hireFreelancer.freelancer}</TableCell>
