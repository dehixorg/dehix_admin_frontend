        ...
        <div className="space-y-5">
          ...
          {(data.githubLink || data.linkedinLink || data.personalWebsite) && (
            <div className="flex items-center space-x-3 pt-2">
              {data.githubLink && (
                <a href={data.githubLink} target="_blank" rel="noopener noreferrer" aria-label="GitHub profile" className="p-2 bg-muted rounded-full hover:bg-gray-200 dark:hover:bg-gray-700 transition-colors">
                  <Github className="h-5 w-5 text-gray-700 dark:text-gray-300" />
                </a>
              )}
              {data.linkedinLink && (
                <a href={data.linkedinLink} target="_blank" rel="noopener noreferrer" aria-label="LinkedIn profile" className="p-2 bg-muted rounded-full hover:bg-blue-100 dark:hover:bg-blue-900/40 transition-colors">
                  <Linkedin className="h-5 w-5 text-blue-600 dark:text-blue-400" />
                </a>
              )}
              {data.personalWebsite && (
                <a href={data.personalWebsite} target="_blank" rel="noopener noreferrer" aria-label="Personal website" className="p-2 bg-muted rounded-full hover:bg-emerald-100 dark:hover:bg-emerald-900/40 transition-colors">
